...
You can do `<Button debug="break" />` to have it break at the beginning of rendering, `<Button debug="verbose" />` to have it output more detailed debug information, or `<Button debug="borders" />` add a border for visual inspection.
